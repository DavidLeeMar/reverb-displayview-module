...
let queryStr = `INSERT INTO sellers (sellername, streetaddress, city, _state, country) VALUES ("${sellername}","${streetaddress}","${city}","${_state}","${country}")`
db.query(queryStr, (err, data)=>{
  if (err) {
    console.log(err)
  } else {
...
var productname = finish + ' ' + faker.commerce.productAdjective() + ' ' +  'Guitar';
...
let queryStr = `INSERT INTO products (productname, price, shippingcost, productdescription, seller_id, category_id, listed, _condition, model, finish, _year, made_in) VALUES ("${productname}", ${price}, ${shippingcost}, "${productdescription}", ${seller_id}, ${category_id}, "${listed}", "${_condition}", "${model}", "${finish}", ${_year}, "${made_in}")`
db.query(queryStr, (err, data)=>{
  if (err) {
    console.log(err)
  } else {
